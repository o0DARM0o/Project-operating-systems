...
static pthread_mutex_t lock_inode_table;
static pthread_mutex_t lock_open_file;
static pthread_mutex_t lock_free_inode;
static pthread_mutex_t lock_free_block;
static pthread_mutex_t lock_file_entries;
static pthread_mutex_t lock_fs_data;
...
static inline bool is_soft_link(inode_t *inode) {
    return inode != NULL && inode->is_softlink;
}
...
int state_init(tfs_params params) {
    ...
    pthread_mutex_init(&(lock_inode_table), NULL);
    pthread_mutex_init(&(lock_file_entries), NULL);
    pthread_mutex_init(&(lock_free_block), NULL);
    pthread_mutex_init(&(lock_free_inode), NULL);
    pthread_mutex_init(&(lock_fs_data), NULL);
    pthread_mutex_init(&(lock_open_file), NULL);
...
static int inode_alloc(void) {
    ...
    pthread_mutex_lock(&lock_free_inode);
    ...
    for (size_t inumber = 0; inumber < INODE_TABLE_SIZE; inumber++) {
        ...
        if (freeinode_ts[inumber] == FREE) {
            ...
            pthread_mutex_unlock(&lock_free_inode);
...
int inode_create(inode_type i_type) {
    ...
    pthread_mutex_lock(&lock_inode_table);
    ...
    case T_DIRECTORY: {
        ...
        inode_table[inumber].cnt_hardlinks = 1;
        inode_table[inumber].is_softlink = 0;
        ...
        for (size_t i = 0; i < MAX_DIR_ENTRIES; i++) {
            pthread_rwlock_init(&dir_entry[i].lock, NULL);
    ...
    case T_SOFTLINK:
        ...
        inode_table[inumber].i_data_block = -1;
        inode_table[inumber].cnt_hardlinks = 1;
        inode_table[inumber].is_softlink = 1;
        pthread_rwlock_init(&(inode_table[inumber].lock), NULL);
        break;
    default:
...
int data_block_alloc(void) {
    pthread_mutex_lock(&lock_free_block);
    ...
    for (size_t i = 0; i < DATA_BLOCKS; i++) {
        ...
        if (free_blocks[i] == FREE) {
            free_blocks[i] = TAKEN;
            pthread_mutex_unlock(&lock_free_block);
            return (int)i;
        }
    }
    pthread_mutex_unlock(&lock_free_block);
...
int add_to_open_file_table(int inumber, size_t offset) {
    ...
    pthread_mutex_lock(&lock_file_entries);
    for (int i = 0; i < MAX_OPEN_FILES; i++) {
        if (free_open_file_entries[i] == FREE) {
            free_open_file_entries[i] = TAKEN;
            open_file_table[i].of_inumber = inumber;
            open_file_table[i].of_offset = offset;
            pthread_mutex_unlock(&lock_file_entries);
            return i;
        }
    }
    pthread_mutex_unlock(&lock_file_entries);
